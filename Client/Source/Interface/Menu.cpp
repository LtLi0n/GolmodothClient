...
Menu::Menu(ConsoleEngine& engine) :
	_engine(engine),
	_button_resume(std::make_unique<Button>(engine, 24, 3)),
	_button_exit(std::make_unique<Button>(engine, 24, 3)),
	_menuActive(false)
{
	_button_resume->OnRender = RenderResumeButton;
	_button_resume->OnClick = [&]() { _menuActive = false; };

	_button_exit->OnRender = RenderExitButton;
	_button_exit->OnClick = []() { exit(0); };
}

bool Menu::MenuActive() const { return _menuActive; }

void Menu::Update()
{
	if (_engine.GetKey(VK_ESCAPE).bPressed) _menuActive = !_menuActive;
